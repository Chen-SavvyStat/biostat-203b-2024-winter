...
```{r}
# Ensure d_labitems_tble is defined correctly for filtering labevents
d_labitems_tble <- tbl(con_bq, "d_labitems") %>%
  filter(itemid %in% c(
...
# Print and summarise labevents_tble
labevents_tble %>%
  summarise(n = n()) %>%
  show_query() %>%
  print()
...
mimic_icu_cohort <- icustays_tble %>%
  ...
  # 计算年龄并筛选成人
  mutate(age_at_intime = as.numeric(anchor_age) + as.numeric(as.Date(intime) %Y% as.Date(anchor_year))) %>%
